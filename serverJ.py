	...
	def do_GET(self):
		...
		if self.path == '/index.html' or self.path == '/home':
			...
			try:
				...
				num_param = query_params.get('num', ['100'])[0]  # Default to 1
				...
				xFr =	subprocess.run(
					...
					f"""bash /wD/Script/MapInputGenB.sh {grep_param} SubName {num_param}; mv input.csv {output_txt}""",
					shell=True
				)
	...
	def do_POST(self):
		...
		if self.path == '/generate-svg':
			...
			try:
				...
				output_txt = f"/tmp/output/output_{os.getpid()}.txt"
				output_svg = f"/tmp/output/output_{os.getpid()}.svg"
				outf_prefix = f"/tmp/output/output_{os.getpid()}"
				with open(upload_file, 'wb') as f:
					f.write(b"dummy content")  # Dummy file content
				...
				xFr =	subprocess.run(
					#f"""Script/MapInputGen.sh '{num_param}' '{grep_param}</Title>' -B 40 /wD/Metadata.xml | grep -e '<CreateDate>' -e '<Extra>' -e '<SubName>' -e '<Title>' -e '<SubType>' > {output_txt}""",
					f"""bash /wD/Script/MapInputGenB.sh {grep_param} {field_param} {num_param}; mv /wD/input.csv {output_txt};python3 /wD/occurrences_map_geopandas.py  {output_txt}  {outf_prefix} {map_title};cp {output_txt} /wD/tmpInput.csv;mv {outf_prefix}_{map_type}.svg  {output_svg};""",
					shell=True
				)
